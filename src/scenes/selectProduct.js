...
botMenu.enter(async (ctx) => {
    ...
    if (BOT === 'Digiflazz') {
        const productOptions = await getProductList(category, brand);
        ctx.session.productOptions = productOptions;
        
        const chunkSize = 8; // compact chunks for mobile
        const chunks = splitList(productOptions, chunkSize);
...
botMenu.on('text', (ctx) => {
    ...
    const BOT = ctx.session.selectedBot;
    if (BOT === 'Digiflazz') {
        const productOptions = ctx.session.productOptions;
        if (!productOptions) {
            ctx.reply('⚠️ Data produk tidak tersedia. Memuat ulang...');
            return ctx.scene.reenter();
        }
        if (!isNaN(text)) {
            const selectedIndex = parseInt(text) - 1;
            if (productOptions[selectedIndex]) {
                ctx.session.selectedProduct = productOptions[selectedIndex];
